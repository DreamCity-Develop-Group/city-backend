		...
		<script type="text/javascript">
            $(document).ready(function () {
                ...
                $("#helpListTable").bootstrapTable({
                    ...
                    columns: [{
						...
						title: "个人所得税",
						field: "earnPersonalTax"
					},{
                        title: "企业所得税",
                        field: "earnEnterpriseTax"
                    },{
                        title: "定额税",
                        field: "earnQuotaTax"
                    },{
                        ...
                        formatter: function (value, row, index) {
							var operateHtml = '';
							if (value === 0){
								operateHtml = '<span class="label label-info">新增</span>';
							}else if (value === 1){
								operateHtml = '<span class="label label-info">收益中</span>';
							}else if (value === 2){
								operateHtml = '<span class="label label-warning">可提取</span>';
							}else if (value === 3){
								operateHtml = '<span class="label label-info">已提取</span>';
							}else if (value === 4){
								operateHtml = '<span class="label label-info">已出局</span>';
							}
                            return operateHtml;
                        }
                    },{
            ...
            function edit(id){
                layer.open({
                    ...
                    shade: false,
                    area: ['800px', '600px'],
                    content: '${ctx}/trade/earning/edit/'  + id,
                    end: function(index){
                        $('#helpListTable').bootstrapTable("refresh");
                    }
                });
            ...
            function del(id){
                layer.confirm('确定删除吗?', {icon: 3, title:'提示'}, function(index){
                    $.ajax({
                        type: "POST",
                        dataType: "json",
                        url: "${ctx}/trade/earning/delete/" + id,
			...
			function detail(id) {
				...
				layer.open({
					...
					shade: false,
					area: ['800px', '600px'],
					content: '${ctx}/trade/earning/get/'  + id,
					end: function(index){
						$('#helpListTable').bootstrapTable("refresh");
					}
				});
