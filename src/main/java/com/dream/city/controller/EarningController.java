 ...
 */
@RestController
@RequestMapping("/trade/earning")
public class EarningController {

    private Logger logger = LoggerFactory.getLogger(this.getClass());

    private final String modelName = "玩家收益";
    private final String actionPath = "trade/earning";
    ...
    public ModelAndView get(@PathVariable("id") Integer id,Model model){
        ...
        PlayerEarning result = null;
        try {
            result = earningService.getEarningById(id);
        }catch (Exception e){
    ...
    public Result getList(Page pageReq, EarningReq record){
        logger.info(modelName + "列表，：{}",record);
        boolean success = Boolean.TRUE;
        PageInfo result = null;
        try{
            pageReq.setCondition(record);
            result = earningService.getEarningList(pageReq,record);
        }catch (Exception e){
